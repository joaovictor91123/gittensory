...
export function resolveThrottledConcurrency(
  ...
  if ((rateLimitRemaining as number) <= lowWaterMark) return 1;
  if ((rateLimitRemaining as number) >= highWaterMark) return baseConcurrency;
  ...
  const fraction = ((rateLimitRemaining as number) - lowWaterMark) / (highWaterMark - lowWaterMark);
