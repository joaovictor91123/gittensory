...
import type { DenyRule, ProposedToolCall } from "./deny-hooks.js";
...
export function buildHouseRulesPreToolUseHook(
  ...
  return async function houseRulesPreToolUseHook(input) {
    try {
      ...
      const verdict = evaluateDenyHooks({ name: toolName, input: toolInput } as ProposedToolCall, rules as DenyRule[]);
